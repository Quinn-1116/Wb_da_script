...
--20014手机号注册的游戏转化率
SELECT t1.date, count(t1.distinct_id) AS `新增用户数`,
                count(t2.distinct_id) AS `登录用户数`,
                count(t2.distinct_id) /count(t1.distinct_id) AS `注册登录转化率`,
                count(t3.distinct_id) AS `触发实名认证用户数`,
                count(t3.distinct_id)/count(t1.distinct_id) AS `实名认证触发率`,
                count(t4.distinct_id) AS `实名认证成功用户数`,
                count(t4.distinct_id)/count(t1.distinct_id) AS `实名认证成功转化率`,
                count(t5.distinct_id) AS `点击进房用户数`,
                count(t5.distinct_id)/count(t1.distinct_id) AS `新用户进房率`,
                count(t6.distinct_id) AS `分配房间用户数`,
                count(t6.distinct_id)/count(t1.distinct_id) AS `新用户匹配率`,
                count(t7.distinct_id) AS`开始游戏用户数`,
                count(t7.distinct_id)/count(t1.distinct_id) AS `新用户游戏率`,
                count(t8.distinct_id) AS`结束游戏用户数`,
                count(t8.distinct_id)/count(t1.distinct_id) AS `新用户完局率`
FROM
  (SELECT date,distinct_id
   FROM events
...
LEFT JOIN
  (SELECT date,distinct_id
   ...
   GROUP BY 1,
            2)t7 ON t1.date = t7.date
AND t1.distinct_id= t7.distinct_id
and t5.distinct_id= t7.distinct_id
AND t6.distinct_id = t7.distinct_id
LEFT JOIN
  (SELECT date,distinct_id
   FROM events
   ...
   GROUP BY 1,
            2)t8 ON t1.date = t8.date
AND t1.distinct_id= t8.distinct_id
AND t5.distinct_id = t8.distinct_id
AND t6.distinct_id = t8.distinct_id
AND t7.distinct_id = t8.distinct_id
GROUP BY 1
ORDER BY 1
...
SELECT t1.date,
       ...
       count(t4.distinct_id)/count(t3.distinct_id) AS `实名认证成功转化率`,
       count(t5.distinct_id) AS `点击进房用户数`,
       count(t5.distinct_id)/count(t3.distinct_id) AS `新用户()进房率`,
       count(t6.distinct_id) AS `分配房间用户数`,
       count(t6.distinct_id)/count(t3.distinct_id) AS `新用户()匹配率`,
       count(t7.distinct_id) AS`开始游戏用户数`,
       count(t7.distinct_id)/count(t3.distinct_id) AS `新用户()游戏率`,
       count(t8.distinct_id) AS`结束游戏用户数`,
       count(t8.distinct_id)/count(t3.distinct_id) AS `新用户()完局率`
FROM
  (SELECT date,distinct_id
   FROM events
   WHERE event = 'register'
