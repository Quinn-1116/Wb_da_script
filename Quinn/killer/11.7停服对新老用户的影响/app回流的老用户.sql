...
SELECT t5.date,
       t5.`当日app回流的受影响的老用户数量`,
       t7.gap,
       t7.`留存用户数`,
       t7.`留存用户数`/ t5.`当日app回流的受影响的老用户数量` AS `retention`
FROM
  (SELECT t3.date,
          count(t3.distinct_id)AS `当日app回流的受影响的老用户数量`
   FROM
     (SELECT t1.date,--停服期间非当日注册的老用户
             t1.distinct_id
      FROM
...
JOIN
  (SELECT t5.date, datediff(t6.date, t5.date)AS gap,
   ...
   FROM
     (SELECT t3.date, t3.distinct_id--当日回流（app）的受到影响的老用户
...
--当天未回流的老用户的留存
SELECT t5.date,
       t5.`当日app未回流的受影响的老用户数量`,
       t7.gap,
       t7.`留存用户数`,
       t7.`留存用户数`/ t5.`当日app未回流的受影响的老用户数量` AS `retention`
FROM
  (SELECT t3.date,
          count(t3.distinct_id)AS `当日app未回流的受影响的老用户数量`
   FROM
     (SELECT t1.date,--停服期间非当日注册的老用户
             t1.distinct_id
      FROM
...
JOIN
  (SELECT t5.date, datediff(t6.date, t5.date)AS gap,
                   count(t6.distinct_id)AS `留存用户数`
   FROM
     (SELECT t3.date, t3.distinct_id--当日未回流（app）的受到影响的老用户

      FROM
        (SELECT t1.date, t1.distinct_id--在停服期间登录的老用户

         FROM
           (SELECT date, distinct_id--停服期间登录的老用户
            FROM events
            WHERE event = 'login'
              AND appId in('20014','30015')
              AND time BETWEEN '2020-11-07 09:00:00' AND '2020-11-07 12:00:00'
